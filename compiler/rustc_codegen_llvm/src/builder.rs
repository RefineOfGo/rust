...
use crate::llvm::{
    self, AtomicOrdering, AtomicRmwBinOp, BasicBlock, False, GetGC, Metadata, ROG_GC_NAME, True,
};
...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn atomic_cmpxchg(
        ...
        let value = unsafe {
            if self.cx.type_kind(ty) == TypeKind::Pointer && !self.can_omit_barriers(dst) {
                let value = self.call_intrinsic("llvm.gcatomic.cas", &[
                    dst,
                    cmp,
                    src,
                    self.cx.const_bool(weak),
                    self.cx.const_bool(false),
                ]);
                llvm::AddCallSiteAttributes(value, llvm::AttributePlace::Function, &[
                    llvm::CreateAttrStringValue(
                        self.cx.llcx,
                        "order",
                        AtomicOrdering::from_generic(order).name(),
    ...
    fn atomic_rmw(
        ...
        unsafe {
            if needs_gcwb {
                let value = self.call_intrinsic("llvm.gcatomic.swap", &[
                    dst,
                    src,
                    self.cx.const_bool(false),
                ]);
