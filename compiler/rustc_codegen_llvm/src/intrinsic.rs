    ...
    fn codegen_intrinsic_call(
        ...
        let llval = match name {
            ...
            sym::get_stack_pointer => {
                let reg = tcx.sess.target.arch.stack_pointer();
                let metadata = self.create_metadata(reg.as_bytes());
                let md_node = self.md_node_in_context(&[metadata]);
                let value = self.get_metadata_value(md_node);
                self.call_intrinsic("llvm.read_register", &[self.type_isize()], &[value])
            }
...
fn catch_unwind_intrinsic<'ll, 'tcx>(
    ...
    if !bx.sess().panic_strategy().unwinds() {
        ...
        codegen_msvc_try(bx, try_func, try_fn_abi, data, catch_func, catch_fn_abi)
    } else if wants_wasm_eh(bx.sess()) {
        codegen_wasm_try(bx, try_func, try_fn_abi, data, catch_func, catch_fn_abi)
    } else {
        codegen_gnu_try(bx, try_func, try_fn_abi, data, catch_func, catch_fn_abi)
    }
}
...
fn get_rust_try_fn<'a, 'll, 'tcx>(
    ...
    if cx.sess().pointer_authentication() {
        ...
        let (_ty, rust_try_fn, _fn_abi) = rust_try;
