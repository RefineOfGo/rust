...
impl<'tcx> ManagedChecker<'tcx> {
    fn is_unmanaged_fast(ty: Ty<'_>) -> bool {
        match ty.kind() {
            ...
            ty::Array(elem_ty, _) | ty::Slice(elem_ty) => Self::is_unmanaged_fast(*elem_ty),
            ty::RawPtr(ty, _) | ty::Ref(_, ty, _) => Self::is_unmanaged_fast(*ty),
            ty::Closure(..)
            | ty::CoroutineClosure(..)
            | ty::Coroutine(..)
        ...
        let is_managed = match self.is_managed_self(ty) {
            ...
            ManagedSelf::Unsure => {
                ...
                let result = match *ty.kind() {
                    ...
                    ty::RawPtr(ty, _) | ty::Ref(_, ty, _) => self.is_managed_impl(ty),
