...
use rustc_abi::{
    AddressSpace, Align, BackendRepr, CanonAbi, ExternAbi, HasDataLayout, HasRegisterMap,
    Primitive, Reg, RegKind, Scalar, Size, TyAbiInterface, TyAndLayout,
};
        ...
        for arg in self.args.iter_mut().chain(iter::once(&mut self.ret)) {
            ...
            match arg.layout.backend_repr {
                BackendRepr::Memory { .. } => {
                    ...
                    let ptr_size = Primitive::Pointer(AddressSpace::DATA).size(cx);
