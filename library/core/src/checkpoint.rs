use crate::intrinsics::{AtomicOrdering, abort, atomic_load, atomic_store_seqcst};
...
pub unsafe fn enable() {
    unsafe { atomic_store_seqcst(&raw mut rog_checkpoint_switch, 1) }
}
...
pub unsafe fn disable() {
    unsafe { atomic_store_seqcst(&raw mut rog_checkpoint_switch, 0) }
}
