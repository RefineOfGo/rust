...
/// requirement, but it uses its own calling convention:
///
/// * x86_64  : passed in the `%r11` register
...
/// registers to clobber, but anyway), so it's the most natural choice.
///
/// Also accessing TLS in AArch64 is not as straightforward as on x86_64, so
/// we also clobber X17 for the MRS instruction that reads from the TPIDR[RO]_EL0
/// register.
///
/// Also naturally, this function MUST preserve every register across the call
