...
#[linkage = "weak"]
#[no_mangle]
extern "C" fn rog_error_handler(_payload: *mut u8) -> bool {
    unreachable!()
}

/// Invoke a closure, capturing the cause of an unwinding panic if one occurs.
#[cfg(not(feature = "panic_immediate_abort"))]
pub unsafe fn r#try_with_rog<R, F: FnOnce() -> R>(f: F) -> Result<R, Box<dyn Any + Send>> {
    union Data<F, R> {
        f: ManuallyDrop<F>,
        r: ManuallyDrop<R>,
    ...
    extern "C" {
        fn rog_error_handler(payload: *mut u8) -> bool;
    }
    ...
    unsafe fn cleanup(payload: *mut u8) -> Box<dyn Any + Send + 'static> {
        if unsafe { rog_error_handler(payload) } {
            unreachable!()
        }
